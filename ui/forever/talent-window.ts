...
import { ForeverTalent, effectiveForeverRank, foreverDefaultOptions, foreverRaces, foreverRaceKeys, foreverClassName, foreverBuildErrors, sampleForeverBuild } from './discovery';
...
export class ForeverTalentWindow {
 ...
 private build(){
  ...
  this.root.append(el('p','ft-legend','✦ New in Forever   ·   ◆ Changed   ·   ↗ Moved   ·   ~ Predicted. Compare highlights changes; inspect a talent for details.'));
 ...
 private tooltip(r:ForeverTalent){
  ...
  const status=ranks.some(rank=>rank.estimated||rank.confidence==='PREDICTED')||r.confidence==='PREDICTED'?'PREDICTED':r.confidence;
