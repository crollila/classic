...
function release() {
  ...
  if (data.engineMode !== 'upstream-baseline') {
    throw new Error('The browser worker currently compiles the Classic baseline. Integrate explicit Forever ruleset selection in the engine adapter before publishing a forever-ruleset release. Metadata alone cannot enable Forever mechanics.');
  }
  ...
  return {
    ...data,
    simulatorVersion: `${JSON.parse(fs.readFileSync(path.join(root, 'package.json'), 'utf8')).version}+${commit}`,
