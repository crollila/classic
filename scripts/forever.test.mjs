...
test('discovery release v2 pins evidence without inventing a beta client build',()=>{
 const fixture=JSON.parse(readFileSync(new URL('../examples/release-discovery-v2.json',import.meta.url),'utf8'));
 assert.equal(validateRelease(fixture).foreverBuild,null);
 assert.throws(()=>validateRelease({...fixture,manifestSha256:null}));
 assert.throws(()=>validateRelease({...fixture,schemaVersion:1}));
 assert.throws(()=>validateRelease({...fixture,mechanicsUpdatedAt:null}));
});
