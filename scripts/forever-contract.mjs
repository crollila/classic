export const CONFIDENCE = ['CONFIRMED', 'PROVISIONAL', 'PREDICTED', 'UNKNOWN', 'HIGH CONFIDENCE', 'EXPERIMENTAL'];
...
export function validateRelease(data) {
  if (!data || ![1,2].includes(data.schemaVersion) || !['upstream-baseline', 'forever-ruleset','forever-discovery'].includes(data.engineMode)) throw new Error('Invalid Forever release metadata');
  if(data.engineMode==='forever-discovery' && (data.schemaVersion!==2 || !data.rulesetId || !/^[a-f0-9]{64}$/.test(data.manifestSha256||'') || !data.mechanicsUpdatedAt))throw new Error('Discovery requires schema v2, ruleset, date and manifest hash; no fictitious client build is required.');
