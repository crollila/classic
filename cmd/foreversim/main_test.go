...
func TestCLIDiscoveryFixtures(t *testing.T) {
	...
	if err = run("forever", "discovery", "../../examples/mage-discovery-invalid-estimate.json", filepath.Join(t.TempDir(), "invalid.json")); err == nil || !strings.Contains(err.Error(), "estimated") {
		t.Fatal("accepted unmarked estimated rank", err)
	}
}
