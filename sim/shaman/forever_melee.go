...
func (s *Shaman) registerForeverStormstrike() {
	...
	s.ForeverDamageMultiplier(func(sp *core.Spell, at *core.AttackTable) float64 {
		if sp.SpellSchool.Matches(core.SpellSchoolNature) && auras.Get(at.Defender).IsActive() {
			return 1.2
		}
		return 1
	})
...
func (s *Shaman) registerForeverUtility() {
	...
	if s.fr("earthbound") > 0 || s.fr("earth-s-grasp") > 0 {
		auras := s.NewEnemyAuraArray(func(t *core.Unit) *core.Aura {
	...
	if s.fr("improved-ghost-wolf") > 0 {
		...
		s.RegisterSpell(core.SpellConfig{ActionID: a.ActionID, Flags: core.SpellFlagAPL | core.SpellFlagHelpful, ManaCost: core.ManaCostOptions{BaseCost: .13}, Cast: core.CastConfig{DefaultCast: core.Cast{GCD: core.GCDDefault, CastTime: 3*time.Second - time.Duration(s.fr("improved-ghost-wolf"))*time.Second}}, ApplyEffects: func(sim *core.Simulation, t *core.Unit, sp *core.Spell) { a.Activate(sim) }})
	}
}
