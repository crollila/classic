...
type AdapterEvidence struct {
	PredictedComponents []string `json:"predicted_components"`
}
type Record struct {
...
func LookupMechanic(id string) (Mechanic, bool) {
	for _, m := range data.Mechanics {
		if m.ID == id {
			m.Adapter.PredictedComponents = slices.Clone(m.Adapter.PredictedComponents)
			return m, true
		}
	}
...
func Validate(p *proto.Player) error {
	...
	if IsStrict(f) && (p.Race == proto.Race_RaceSkyborneWindshaper || p.Race == proto.Race_RaceSkyborneHighOrder) {
		return fmt.Errorf("Skyborne base stats are predicted; choose an established race in STRICT mode")
	}
	...
	for _, id := range f.Mechanics {
		...
		seen[id] = true
		m, _ := LookupMechanic(id)
		if _, isClass := data.ClassicLayout[m.Category]; isClass && m.Category != ClassName(p.Class) {
			return fmt.Errorf("wrong-class Forever mechanic %s", id)
		}
...
func Prepare(p *proto.Player) (*proto.Player, error) {
	...
	if IsStrict(p.Forever) {
		for id, n := range out.Forever.MechanicRanks {
			if n > 1 {
				out.Forever.MechanicRanks[id] = 1
			}
		}
	}
