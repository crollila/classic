...
func (c *Character) applyForeverTalents() {
	...
	for _, id := range ids {
		...
		case "druid.talent.moonfury":
			c.OnSpellRegistered(func(s *Spell) {
				if s.SpellSchool.Matches(SpellSchoolArcane | SpellSchoolNature) {
...
func (c *Character) applyForeverBuffs(raid *proto.RaidBuffs) {
	...
	// Scenario assumption: the recorded campsite buff is already active throughout
	// the encounter. Duration, camping activation and the 1-hour cooldown are not
	// simulated. Exclusivity follows the existing Fortitude strength comparison.
