...
func TestForeverInterruptedSchoolIsolation(t *testing.T) {
	sim, c := reviewSim(t)
	var fire, frost *core.Spell
	for _, s := range c.Spellbook {
		if s.DefaultCast.CastTime > 0 && s.Flags.Matches(core.SpellFlagAPL) {
			if s.SpellSchool == core.SpellSchoolFire {
				fire = s
			}
			if s.SpellSchool == core.SpellSchoolFrost {
				frost = s
			}
		}
	}
	if fire == nil || frost == nil {
		t.Fatal("missing castable school fixtures")
	}
	if c.ForeverInterruptSchool(sim, 4*time.Second) {
		t.Fatal("idle target was interrupted")
	}
	c.Hardcast = core.Hardcast{ActionID: fire.ActionID, Expires: sim.CurrentTime + 2*time.Second}
	if !c.ForeverInterruptSchool(sim, 4*time.Second) {
		t.Fatal("active cast not interrupted")
	}
	if fire.CanCast(sim, c.CurrentTarget) || !frost.CanCast(sim, c.CurrentTarget) {
		t.Fatal("lockout must affect only the interrupted school")
	}
	reviewAdvance(t, sim, 5*time.Second)
	if !fire.CanCast(sim, c.CurrentTarget) {
		t.Fatal("school did not unlock")
	}
	c.Hardcast = core.Hardcast{ActionID: fire.ActionID, Expires: sim.CurrentTime + 2*time.Second}
	c.GetAura("review-interrupt-resistance").Activate(sim)
	if c.ForeverInterruptSchool(sim, 4*time.Second) || c.Hardcast.Expires <= sim.CurrentTime {
		t.Fatal("interrupt resistance did not preserve cast")
	}
	c.GetAura("review-interrupt-resistance").Deactivate(sim)
	c.ForeverInterruptSchool(sim, 4*time.Second)
	sim.Cleanup()
	sim.Reset()
	if !fire.CanCast(sim, c.CurrentTarget) {
		t.Fatal("school lockout survived reset")
	}
}
