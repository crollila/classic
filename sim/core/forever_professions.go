...
func (c *Character) applyForeverProfessions() {
	...
	if c.HasForeverMechanic("profession.blacksmithing.belt-buckles") && c.HasProfession(proto.Profession_Blacksmithing) && c.Equipment[proto.ItemSlot_ItemSlotWaist].ID != 0 {
		...
		if c.Class == proto.Class_ClassHunter || c.Class == proto.Class_ClassRogue || c.Spec == proto.Spec_SpecFeralDruid || c.Spec == proto.Spec_SpecFeralTankDruid {
			attribute = stats.Agility
		} else if c.Class == proto.Class_ClassMage || c.Class == proto.Class_ClassWarlock || c.Class == proto.Class_ClassPriest || c.Spec == proto.Spec_SpecBalanceDruid || c.Spec == proto.Spec_SpecRestorationDruid || c.Spec == proto.Spec_SpecHolyPaladin || c.Spec == proto.Spec_SpecElementalShaman || c.Spec == proto.Spec_SpecRestorationShaman {
