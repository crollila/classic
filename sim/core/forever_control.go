...
func (u *Unit) ForeverInterrupt(sim *Simulation) {
	if u.foreverResistsInterrupt(sim) { return }
...
func (u *Unit) ForeverInterruptSchool(sim *Simulation, duration time.Duration) bool {
	...
	if interrupted == nil || u.foreverResistsInterrupt(sim) { return false }
	u.foreverInterruptCast(sim)
	if duration <= 0 { return true }
	state := u.initForeverControl()
	if state.schoolLocks == nil { state.schoolLocks = map[*Aura]SpellSchool{} }
	...
	if a.IsActive() { duration = max(duration, a.RemainingDuration(sim)) }
	a.Duration = duration
	a.Activate(sim)
	return true
}
