			...
			if s.Discovery {
				...
				for id, n := range player.Forever.Talents {
					if n > 0 {
						...
						rank := r.Ranks[n-1]
						metadata.TalentEvidence = append(metadata.TalentEvidence, TalentEvidence{partyIndex*5 + playerIndex, id, n, rank.Confidence, rank.Estimated, rank.SourceURL, "PROVISIONAL"})
					}
				}
				for _, id := range player.Forever.Mechanics {
					seen[id] = true
				}
				metadata.Experimental = metadata.Experimental || player.Forever.ExperimentalEstimatedRanks
			}
		}
	}
	for id := range seen {
