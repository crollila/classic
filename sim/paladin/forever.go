...
func (p *Paladin) applyForeverTalents() {
	...
	p.OnSpellRegistered(func(sp *core.Spell) {
		...
		// Instrument of Law first: a Hammer of Wrath it makes instant is an instant spell
		// for Benediction.
		if sp.SpellCode == SpellCode_PaladinHammerOfWrath {
			...
			// Hammer of Wrath is a Holy spell on the ranged table, which the magic-only
			// Holy Power / Divine Precision hooks miss.
			sp.BonusCritRating += p.fr("holy-power") * core.CritRatingPerCritChance
			sp.BonusHitRating += p.ForeverValue("paladin.talent.divine-precision", 0, 0) * core.MeleeHitRatingPerHitChance
		}
		// The Judgement dummy rolls the spell hit table for Judgement of Command and has no
		// defense type, so the Holy-school hook skips it too.
		if sp.ActionID.SpellID == 20271 {
			sp.BonusHitRating += p.ForeverValue("paladin.talent.divine-precision", 0, 0) * core.SpellHitRatingPerHitChance
		}
		...
		if sp.SpellCode == SpellCode_PaladinExorcism || sp.SpellCode == SpellCode_PaladinHolyWrath {
			sp.CD.Duration = time.Duration(float64(sp.CD.Duration) * (1 - p.ForeverValue("paladin.talent.purifying-power", 1, 0)/100))
		}
	...
	if p.fr("vindication") > 0 {
		...
		enemies := p.NewEnemyAuraArray(func(t *core.Unit) *core.Aura {
			v := []float64{0, 67, 133, 200}[int(p.fr("vindication"))] // client tooltip
		...
		core.MakePermanent(p.RegisterAura(core.Aura{Label: "Forever Vindication Trigger", OnSpellHitDealt: func(_ *core.Aura, sim *core.Simulation, sp *core.Spell, r *core.SpellResult) {
			// "Your damaging melee attacks": swings and melee abilities that deal damage, not
			// seal or item procs. The client gives no proc chance (ElliotWood/Forever reads 100%).
			if r.Landed() && r.Damage > 0 && sp.ProcMask.Matches(core.ProcMaskMelee) && !sp.ProcMask.Matches(core.ProcMaskProc) {
				a.Activate(sim)
	...
	if p.fr("twist-of-light") > 0 {
		p.foreverEcho = p.RegisterAura(core.Aura{Label: "Forever Seal Echo", ActionID: p.fa("twist-of-light"), Duration: core.NeverExpires, OnSpellHitDealt: func(a *core.Aura, sim *core.Simulation, sp *core.Spell, r *core.SpellResult) {
			// The next melee attack, not a seal or item proc riding on it.
			if !sp.ProcMask.Matches(core.ProcMaskMelee) || sp.ProcMask.Matches(core.ProcMaskProc) {
				return
			}
...
func (p *Paladin) registerForeverSpells() {
	...
	// Holy Strike is a Forever trainer spell from level 6 (8 ranks, client tooltips in
	// level_ranks.go): a share of normalized weapon damage plus a flat Holy amount, 12 sec
	// cooldown, 0.429 spell power (ElliotWood/Forever reads that coefficient from the client).
	...
	if i := p.foreverRankIndex("Holy Strike", false); i >= 0 {
		rank := foreverHolyStrikeRanks[i]
		p.foreverHolyStrike = p.RegisterSpell(core.SpellConfig{ActionID: p.ForeverAction("paladin.baseline.holy-strike"), SpellCode: foreverHolyStrike, SpellSchool: core.SpellSchoolHoly, DefenseType: core.DefenseTypeMelee, ProcMask: core.ProcMaskMeleeMHSpecial, Flags: core.SpellFlagAPL | core.SpellFlagMeleeMetrics, Rank: i + 1, ManaCost: core.ManaCostOptions{FlatCost: rank.mana}, Cast: core.CastConfig{DefaultCast: core.Cast{GCD: core.GCDDefault}, IgnoreHaste: true, CD: core.Cooldown{Timer: p.NewTimer(), Duration: 12*time.Second - time.Duration(p.fr("improved-holy-strike"))*time.Second}}, DamageMultiplier: 1 + .1*p.fr("sacred-arbiter"), ThreatMultiplier: 1 + .05*p.fr("iron-creed"), BonusCoefficient: .429, ApplyEffects: func(sim *core.Simulation, t *core.Unit, sp *core.Spell) {
			base := rank.weapon*p.MHNormalizedWeaponDamage(sim, sp.MeleeAttackPower(t)) + sim.Roll(rank.low, rank.high)
			r := sp.CalcAndDealDamage(sim, t, base, sp.OutcomeMeleeSpecialHitAndCrit)
			if r.Landed() {
				if p.Options.RighteousFury {
					iron.Activate(sim)
				}
	...
	if p.fr("sanctified-judgement") > 0 {
		...
		p.judgement.ApplyEffects = func(sim *core.Simulation, t *core.Unit, sp *core.Spell) {
			...
			if sim.Proc(p.ForeverValue("paladin.talent.sanctified-judgement", 0, 0)/100, "Forever Sanctified Judgement") {
				// 20/40/60% of the judged seal's cost (client tooltip, value index 1).
				p.AddMana(sim, p.ForeverValue("paladin.talent.sanctified-judgement", 1, 0)/100*cost, metrics)
			}
		}
	}
}
...
func (p *Paladin) registerForeverHealing() {
	...
	// Holy Shock and Light's Vigil are talent spells whose later ranks are trained: the
	// talent grants rank 1, the level picks the highest trained rank (client tooltips).
	vigilRank := foreverLightsVigilRanks[max(0, p.foreverRankIndex("Light's Vigil", true))]
	shockRank := foreverHolyShockRanks[max(0, p.foreverRankIndex("Holy Shock", true))]
	if p.fr("light-s-vigil") > 0 {
		...
		// from Holy Shock. The rank's Mana cost is retained for Illumination;
	...
	if p.fr("holy-shock") > 0 {
		...
		shock = p.RegisterSpell(core.SpellConfig{ActionID: p.fa("holy-shock"), SpellCode: SpellCode_PaladinHolyShock, SpellSchool: core.SpellSchoolHoly, DefenseType: core.DefenseTypeMagic, ProcMask: core.ProcMaskSpellDamage | core.ProcMaskSpellHealing, Flags: core.SpellFlagAPL, ManaCost: core.ManaCostOptions{FlatCost: shockRank.mana}, Cast: core.CastConfig{DefaultCast: core.Cast{GCD: core.GCDDefault}, CD: core.Cooldown{Timer: p.NewTimer(), Duration: 10 * time.Second}}, DamageMultiplier: 1, ThreatMultiplier: 1, BonusCoefficient: .429, ApplyEffects: func(sim *core.Simulation, t *core.Unit, sp *core.Spell) {
			...
			if a := vigils[t]; a != nil && a.IsActive() {
				...
				if p.IsOpponent(t) {
					p.AddMana(sim, vigilRank.mana*.75, metrics)
				}
			}
		}})
	}
	if shock != nil {
		p.ForeverSpellRange(shock, 40) // client: 40 yd (Classic 20)
	}
